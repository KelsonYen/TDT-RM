...
| `options.csv` | TAIFEX `PutCallRatio` 與 `TAIFEXVIX` OpenAPI | **原始資料可自動抓，但 production 欄位尚未自動化**。Adapter 可取 PCR、put/call volume、TAIFEX VIX。 | Strict schema 需要 `pcr_stable`、`pcr_rises`、`vix_stable`、`vix_rises`、`tail_risk`；目前 parser 未定義 PCR/VIX 穩定或上升門檻，也沒有 formal Tail Risk source; BCD is computed internally；public manifest 已指出未供應 formal scores 時 pipeline 只能用既有 fallback 行為。 | 定義 PCR/VIX lookback 與門檻；串接 formal Tail Risk 或明確標記 deterministic provisional scores；將 options raw data 轉成 strict booleans + scores；補 score provenance 與禁止 mock/fallback。 |
